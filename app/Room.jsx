...
export function Room({ children,params }) {
  return (
    ...
    resolveUsers={async ({ userIds }) => {
      const q = query(collection(db, 'LoopUser'), where('email', 'in', userIds));
      ...
      querySnapshot.forEach((doc) => {
        ...
        userList.push({
          ...userData,
          avatar: userData.avatar || defaultAvatarUrl, // Ensure avatar URL is included
        });
      });
      return userList;
    }}
    ...
    resolveMentionSuggestions={async ({ text, roomId }) => {
      ...
      const q=query(collection(db,'LoopUser'),where('email','!=',null));
      const querySnapshot=await getDocs(q);
      let userList=[];
      querySnapshot.forEach((doc)=>{
